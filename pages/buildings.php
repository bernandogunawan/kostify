...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if ($action == 'add') {
        ...
        mysqli_query($conn, "INSERT INTO building (name, address, city, total_floors, description)
                             VALUES ('$name','$address','$city',$floors,'$desc')");
        ...
        if ($rooms > 0) {
            $error = 'Cannot delete: building has ' . $rooms . ' room(s) assigned.';
        } else {
...
$total_cities    = mysqli_fetch_assoc(mysqli_query($conn, "SELECT COUNT(DISTINCT city) as c FROM building"))['c'];
?>
<!DOCTYPE html>
<html lang="en">
<head>
    ...
    <style>
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .building-card:hover {
            box-shadow: 0 8px 32px rgba(28,20,16,.10);
        ...
        .building-card-header::before {
            content: '';
            position: absolute;
            top: -40px; right: -40px;
            width: 140px; height: 140px;
            background: radial-gradient(circle, rgba(181,85,106,.35) 0%, transparent 70%);
            border-radius: 50%;
        }

        .building-card-header::after {
            content: '🏢';
            position: absolute;
        ...
        .bstat-num.occ { color: var(--rose); }
        .bstat-num.avail { color: #2E7D32; }
        ...
        .btn-edit {
            background: rgba(181,85,106,.1);
            color: var(--rose);
        }

        .btn-edit:hover { background: var(--rose); color: #fff; }
        ...
        .btn-view:hover { background: var(--dark); color: #fff; }
        ...
        .form-group textarea:focus { border-color: var(--rose); box-shadow: 0 0 0 3px rgba(181,85,106,.1); }
        ...
        .modal-danger .modal-header { border-bottom-color: #FFCDD2; }
        .modal-danger .modal-header h3 { color: #C62828; }
...
<div class="main">
    ...
    <div class="topbar">
        <div>
            <h1>Buildings</h1>
            <p>Manage all properties and locations</p>
        </div>
    ...
    <div class="toolbar">
        <div class="search-wrap">
            ...
            <input type="text" id="searchInput" placeholder="Search buildings…" oninput="filterCards()">
        </div>
        <select class="filter-select" id="cityFilter" onchange="filterCards()">
            <option value="">All Cities</option>
            <?php
            $cities = mysqli_query($conn, "SELECT DISTINCT city FROM building ORDER BY city");
    ...
    while ($b = mysqli_fetch_assoc($buildings)):
        $has_buildings   = true;
        $occ_pct         = $b['total_rooms'] > 0 ? round($b['occupied'] / $b['total_rooms'] * 100) : 0;
        $safe_name       = htmlspecialchars($b['name']);
        $safe_address    = htmlspecialchars($b['address']);
        $safe_city       = htmlspecialchars($b['city']);
        $safe_desc       = htmlspecialchars($b['description'] ?? '');
    ?>
    <div class="building-card"
         data-name="<?= strtolower($safe_name) ?>"
         data-city="<?= strtolower($safe_city) ?>">

        <div class="building-card-header">
            <div class="building-name"><?= $safe_name ?></div>
            <div class="building-city"><?= $safe_city ?></div>
        </div>
        ...
        <div class="building-card-body">
            ...
            <div class="building-stats">
                <div class="bstat">
                    <div class="bstat-num"><?= $b['total_rooms'] ?></div>
                    <div class="bstat-lbl">Total</div>
                </div>
                <div class="bstat">
                    <div class="bstat-num occ"><?= $b['occupied'] ?></div>
                    <div class="bstat-lbl">Occupied</div>
                </div>
                <div class="bstat">
                    <div class="bstat-num avail"><?= $b['available'] ?></div>
                    <div class="bstat-lbl">Available</div>
                </div>
            </div>

            <div class="occupancy-bar">
                <div class="occupancy-fill" style="width:<?= $occ_pct ?>%"></div>
            </div>

            <?php if ($safe_desc): ?>
            <div class="building-desc"><?= $safe_desc ?></div>
            <?php endif ?>
        </div>

        <div class="building-card-footer">
            <button class="btn-action btn-view"
                    onclick="viewBuilding(<?= $b['building_id'] ?>)">
                🔍 Rooms
            </button>
            <button class="btn-action btn-edit"
                    onclick='openEditModal(<?= $b["building_id"] ?>, <?= json_encode($b["name"]) ?>, <?= json_encode($b["address"]) ?>, <?= json_encode($b["city"]) ?>, <?= (int)$b["total_floors"] ?>, <?= json_encode($b["description"] ?? "") ?>)'>
                ✏️ Edit
            </button>
            <button class="btn-action btn-delete"
                    onclick='openDeleteModal(<?= $b["building_id"] ?>, <?= json_encode($b["name"]) ?>)'>
                🗑️ Delete
            </button>
        </div>
    </div>
        ...
        <form method="POST">
            ...
            <div class="modal-body">
                <div class="form-group">
                    ...
                    <input type="text" name="name" placeholder="e.g. Kost Melati" required>
                </div>
                <div class="form-row">
                    <div class="form-group">
                        <label>City</label>
                    ...
                    <div class="form-group">
                        ...
                        <input type="number" name="floors" min="1" max="99" placeholder="3" required>
                    </div>
                </div>
                <div class="form-group">
                    <label>Address</label>
                    <input type="text" name="address" placeholder="Jl. Kebon Jeruk No. 12" required>
                </div>
                <div class="form-group">
                    <label>Description <span style="text-transform:none;letter-spacing:0;font-weight:400">(optional)</span></label>
                    <textarea name="description" placeholder="Brief description of the building…"></textarea>
                </div>
            </div>
            <div class="modal-footer">
                <button type="button" class="btn-cancel" onclick="closeModal('addModal')">Cancel</button>
                <button type="submit" class="btn-submit">＋ Add Building</button>
            </div>
        </form>
    </div>
</div>
...
<div class="modal-backdrop" id="editModal">
    <div class="modal">
        ...
        <form method="POST">
            ...
            <input type="hidden" name="building_id" id="editId">
            <div class="modal-body">
                <div class="form-group">
                    <label>Building Name</label>
                ...
                <div class="form-row">
                    ...
                    <div class="form-group">
                        <label>Total Floors</label>
                        <input type="number" name="floors" id="editFloors" min="1" max="99" required>
                    </div>
                </div>
                <div class="form-group">
                    <label>Address</label>
...
<div class="modal-backdrop" id="deleteModal">
    <div class="modal modal-danger">
        ...
        <form method="POST">
            ...
            <input type="hidden" name="building_id" id="deleteId">
            <div class="modal-body">
                <div class="delete-warning">
                    <div class="warning-icon">⚠️</div>
                    <p>You're about to delete <strong id="deleteName"></strong>.<br>This action cannot be undone. All associated data may be affected.</p>
                </div>
            </div>
            <div class="modal-footer">
                <button type="button" class="btn-cancel" onclick="closeModal('deleteModal')">Cancel</button>
                <button type="submit" class="btn-danger">🗑️ Delete Building</button>
            </div>
        </form>
    </div>
</div>
...
<script>
    ...
    function viewBuilding(id) {
        window.location.href = 'rooms.php?building_id=' + id;
    }
    ...
    function filterCards() {
        ...
        cards.forEach(card => {
            ...
            const show = (!query || name.includes(query)) && (!city || c === city);
