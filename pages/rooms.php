...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if ($action == 'add') {
        ...
        // Check duplicate room number in same building
        ...
        if ($used > 0) {
            $error = 'Cannot delete: room has ' . $used . ' booking(s).';
        } else {
...
$rooms = mysqli_query($conn, "
    SELECT r.*, b.name as building_name, b.city
    FROM room r
...
$occupied       = mysqli_fetch_assoc(mysqli_query($conn, "SELECT COUNT(*) as c FROM room WHERE status='Occupied'"))['c'];
$available      = mysqli_fetch_assoc(mysqli_query($conn, "SELECT COUNT(*) as c FROM room WHERE status='Available'"))['c'];
$total_rev      = mysqli_fetch_assoc(mysqli_query($conn, "SELECT COALESCE(SUM(price_per_month),0) as s FROM room WHERE status='Occupied'"))['s'];

// ── Fetch buildings for selects ──
$buildings = mysqli_query($conn, "SELECT building_id, name, city FROM building ORDER BY name");
$buildings_arr = [];
while ($bld = mysqli_fetch_assoc($buildings)) $buildings_arr[] = $bld;

// ── Distinct types for filter ──
$types_res = mysqli_query($conn, "SELECT DISTINCT room_type FROM room ORDER BY room_type");
$types = [];
...
<head>
    ...
    <style>
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .stat-value { font-size: 24px; font-weight: 700; color: var(--dark); margin-top: 2px; }
        ...
        .modal-close { width: 32px; height: 32px; background: #F0EBE1; border: none; border-radius: 50%; font-size: 16px; cursor: pointer; display: flex; align-items: center; justify-content: center; transition: background .2s; }
        ...
        .form-group input, .form-group select, .form-group textarea { width: 100%; padding: 11px 14px; border: 1.5px solid var(--border); border-radius: 9px; font-family: 'DM Sans', sans-serif; font-size: 14px; color: var(--dark); background: #FDFAF6; outline: none; transition: border-color .2s; }
        .form-group input:focus, .form-group select:focus, .form-group textarea:focus { border-color: var(--rose); box-shadow: 0 0 0 3px rgba(181,85,106,.1); }
        ...
        .btn-cancel { padding: 10px 22px; background: #F0EBE1; border: 1.5px solid var(--border); border-radius: 9px; font-family: 'DM Sans', sans-serif; font-size: 14px; font-weight: 500; color: var(--muted); cursor: pointer; transition: all .2s; }
        .btn-cancel:hover { background: var(--border); color: var(--dark); }
        .btn-submit { padding: 10px 24px; background: var(--rose); border: none; border-radius: 9px; font-family: 'DM Sans', sans-serif; font-size: 14px; font-weight: 600; color: #fff; cursor: pointer; transition: background .2s; }
        .btn-submit:hover { background: var(--rose-dark); }
        .btn-danger { padding: 10px 24px; background: #C62828; border: none; border-radius: 9px; font-family: 'DM Sans', sans-serif; font-size: 14px; font-weight: 600; color: #fff; cursor: pointer; transition: background .2s; }
...
<div class="main">
    <div class="topbar">
        <div>
            <h1>Rooms</h1>
            <p>Manage all rooms across your properties</p>
        </div>
    ...
    <div class="stats">
        ...
        <div class="stat-card"><div class="stat-icon">💰</div><div><div class="stat-label">Monthly Revenue</div><div class="stat-value" style="font-size:18px">Rp <?= number_format($total_rev, 0, ',', '.') ?></div></div></div>
    </div>
    ...
    <div class="toolbar">
        <div class="search-wrap">
            ...
            <input type="text" id="searchInput" placeholder="Search room number…" oninput="filterTable()">
        </div>
        <select class="filter-select" id="buildingFilter" onchange="filterTable()">
            <option value="">All Buildings</option>
            <?php foreach ($buildings_arr as $bld): ?>
                <option value="<?= htmlspecialchars($bld['name']) ?>" <?= ($filter_building && $bld['building_id'] == $filter_building) ? 'selected' : '' ?>>
        ...
        <div class="table-wrap">
            <table id="roomTable">
                ...
                while ($r = mysqli_fetch_assoc($rooms)):
                    $has_rows = true;
                    $badge_class = 'badge-' . strtolower(str_replace(' ', '', $r['status']));
                ?>
                <tr data-room="<?= strtolower($r['room_number']) ?>"
                    ...
                    <td>
                        <div><?= htmlspecialchars($r['building_name']) ?></div>
                        <div class="building-tag"><?= htmlspecialchars($r['city']) ?></div>
                    </td>
                    <td><span class="type-chip"><?= htmlspecialchars($r['room_type']) ?></span></td>
                    <td>Floor <?= $r['floor'] ?></td>
                    <td><?= $r['size'] ? htmlspecialchars($r['size']) : '—' ?></td>
                    <td class="price">Rp <?= number_format($r['price_per_month'], 0, ',', '.') ?></td>
                    <td><span class="badge <?= $badge_class ?>"><?= htmlspecialchars($r['status']) ?></span></td>
                    <td>
                        <div class="actions">
                ...
                <?php endwhile ?>
                <?php if (!$has_rows): ?>
                    <tr class="empty-row"><td colspan="9">🚪 No rooms found. Add your first room!</td></tr>
                <?php endif ?>
                </tbody>
            </table>
        </div>
        <div class="table-footer">
            <span id="rowCount">Showing all rooms</span>
        </div>
    </div>
</div>
            ...
            <div class="modal-body">
                <div class="form-group">
                    ...
                    <select name="building_id" required>
                        ...
                        <?php foreach ($buildings_arr as $bld): ?>
                            <option value="<?= $bld['building_id'] ?>"><?= htmlspecialchars($bld['name']) ?> (<?= htmlspecialchars($bld['city']) ?>)</option>
                        <?php endforeach ?>
                    </select>
                </div>
                <div class="form-row">
                    ...
                    <div class="form-group">
                        <label>Floor</label>
                        <input type="number" name="floor" min="1" placeholder="1" required>
                    </div>
                </div>
                <div class="form-row">
                    <div class="form-group">
                    ...
                    <div class="form-group">
                        <label>Size (m²)</label>
                        <input type="text" name="size" placeholder="e.g. 12m²">
                    </div>
                </div>
                <div class="form-row">
                    <div class="form-group">
                        <label>Price / Month (Rp)</label>
                        <input type="number" name="price_per_month" placeholder="1500000" required>
                    </div>
                    <div class="form-group">
                        <label>Status</label>
                ...
                <div class="form-group">
                    <label>Facilities <span style="text-transform:none;letter-spacing:0;font-weight:400">(optional)</span></label>
                    <input type="text" name="facilities" placeholder="e.g. AC, WiFi, Private Bathroom">
                </div>
            </div>
        ...
        <form method="POST">
            ...
            <div class="modal-body">
                ...
                <div class="form-row">
                    ...
                    <div class="form-group">
                        <label>Floor</label>
                        <input type="number" name="floor" id="editFloor" min="1" required>
                    </div>
                </div>
                <div class="form-row">
                    <div class="form-group">
                ...
                <div class="form-row">
                    <div class="form-group">
                        <label>Price / Month (Rp)</label>
                        <input type="number" name="price_per_month" id="editPrice" required>
                    </div>
                    <div class="form-group">
                        <label>Status</label>
...
<div class="modal-backdrop" id="deleteModal">
    <div class="modal">
        <div class="modal-header">
            <h3 style="color:#C62828">Delete Room</h3>
        ...
        <form method="POST">
            ...
            <div class="modal-body">
                <div class="delete-warning">
                    ...
                    <p>You're about to delete Room <strong id="deleteRoomNum"></strong>.<br>This cannot be undone.</p>
                </div>
            </div>
            <div class="modal-footer">
...
function filterTable() {
    ...
    let shown = 0;
    rows.forEach(row => {
        const match = (!q      || row.dataset.room.includes(q))
                   && (!bld    || row.dataset.building.toLowerCase() === bld)
