...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if ($action == 'add') {
        ...
        if ($dup) {
            ...
            mysqli_query($conn, "INSERT INTO tenant (first_name,last_name,email,phone,password)
        ...
        // Check email dup on another tenant
        ...
        if ($bookings > 0) {
            $error = 'Cannot delete: tenant has ' . $bookings . ' booking(s).';
        } else {
            mysqli_query($conn, "DELETE FROM tenant WHERE tenant_id=$id");
            $success = 'Tenant deleted.';
        }
    }
}

// ── Fetch tenants with booking info ──
$search = mysqli_real_escape_string($conn, $_GET['q'] ?? '');
$where  = $search ? "WHERE (t.first_name LIKE '%$search%' OR t.last_name LIKE '%$search%' OR t.email LIKE '%$search%')" : '';

$tenants = mysqli_query($conn, "
    SELECT t.*,
           COUNT(b.booking_id)                              AS total_bookings,
           MAX(b.booking_date)                              AS last_booking,
           (SELECT r.room_number FROM booking b2
            JOIN room r ON b2.room_id = r.room_id
            WHERE b2.tenant_id = t.tenant_id
              AND b2.status = 'Active'
            LIMIT 1)                                        AS current_room
    FROM tenant t
    LEFT JOIN booking b ON b.tenant_id = t.tenant_id
    $where
...
$active_tenants = mysqli_fetch_assoc(mysqli_query($conn, "
    SELECT COUNT(DISTINCT b.tenant_id) as c FROM booking b WHERE b.status='Active'
"))['c'];
$new_this_month = mysqli_fetch_assoc(mysqli_query($conn, "
    SELECT COUNT(*) as c FROM tenant WHERE MONTH(created_at)=MONTH(NOW()) AND YEAR(created_at)=YEAR(NOW())
"))['c'] ?? 0;
?>
<!DOCTYPE html>
<html lang="en">
...
<div class="main">
    ...
    <div class="stats">
        ...
        <div class="stat-card"><div class="stat-icon">🏠</div><div><div class="stat-label">Currently Staying</div><div class="stat-value"><?= $active_tenants ?></div></div></div>
        <div class="stat-card"><div class="stat-icon">🆕</div><div><div class="stat-label">New This Month</div><div class="stat-value"><?= $new_this_month ?></div></div></div>
    </div>
    ...
    <div class="table-card">
        <div class="table-wrap">
            <table id="tenantTable">
                ...
                <tr data-search="<?= strtolower($t['first_name'].' '.$t['last_name'].' '.$t['email']) ?>">
                    ...
                    <td>
                        <div class="t-info">
                            ...
                            <div>
                                <div class="t-name"><?= htmlspecialchars($t['first_name'].' '.$t['last_name']) ?></div>
                                <div class="t-email"><?= htmlspecialchars($t['email']) ?></div>
                            </div>
                        </div>
                    </td>
                    <td><?= htmlspecialchars($t['phone'] ?: '—') ?></td>
                    <td>
                        <?php if ($t['current_room']): ?>
                            <span class="badge badge-active">Room <?= htmlspecialchars($t['current_room']) ?></span>
                        <?php else: ?>
                            <span class="badge badge-no-room">No Active Room</span>
                        <?php endif ?>
                    </td>
                ...
                <?php endwhile ?>
                <?php if (!$has_rows): ?>
                    <tr class="empty-row"><td colspan="7">👥 No tenants found.</td></tr>
                <?php endif ?>
                </tbody>
            </table>
        </div>
        <div class="table-footer">
            <span id="rowCount"><?= $total_tenants ?> tenant(s) total</span>
        </div>
    </div>
</div>
        ...
        <form method="POST">
            ...
            <div class="modal-body">
                ...
                <div class="form-group">
                    ...
                    <div class="form-hint">Tenant will use this to log in to the portal.</div>
                </div>
            </div>
    ...
    <div class="modal">
        ...
        <form method="POST">
            ...
            <div class="modal-body">
                ...
                <div class="form-group">
                    <label>New Password <span style="text-transform:none;letter-spacing:0;font-weight:400">(leave blank to keep current)</span></label>
        ...
        <form method="POST">
            ...
            <div class="modal-body">
                <div class="delete-warning">
                    ...
                    <p>You're about to delete <strong id="deleteTenantName"></strong>.<br>This cannot be undone.</p>
                </div>
            </div>
            <div class="modal-footer">
                <button type="button" class="btn-cancel" onclick="closeModal('deleteModal')">Cancel</button>
                <button type="submit" class="btn-danger">🗑️ Delete Tenant</button>
            </div>
        </form>
    </div>
</div>
...
function openEditModal(t) {
    document.getElementById('editTenantId').value = t.tenant_id;
    document.getElementById('editFirst').value    = t.first_name;
    document.getElementById('editLast').value     = t.last_name;
